...
export function AuthForm({ mode }: { mode: 'login' | 'register' | 'forgot' }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) return setMessage(json.error ?? 'Não foi possível continuar.');
    if (mode === 'forgot') return setMessage(json.data.message);
  ...
  return <form onSubmit={submit} className="mx-auto w-full max-w-md rounded-3xl border border-zinc-200 bg-white p-8 shadow-soft dark:border-zinc-800 dark:bg-zinc-900">
    ...
    <button className="mt-6 w-full rounded-xl bg-atto-red px-5 py-3 font-bold text-white">Continuar</button>
  </form>;
}
